...
class ApiService {
  private async request<T>(
    ...
    try {
      ...
      if (error.message === 'Failed to fetch' || error.name === 'TypeError') {
        throw new Error(`Failed to connect to backend server. Please ensure the server is running at ${API_BASE_URL}`);
      }
      throw error;
    }
  }
  ...
  async analyzeFiles(files: File[]): Promise<AnalysisResponse> {
    console.log('[ApiService] analyzeFiles called with', files.length, 'files');
    
    const formData = new FormData();
    files.forEach((file, index) => {
      console.log(`[ApiService] Adding file ${index + 1}: ${file.name}, size: ${file.size}`);
      formData.append('files', file);
    });

    console.log(`[ApiService] Sending request to ${API_BASE_URL}/analysis/analyze`);
    ...
    try {
      ...
      console.log('[ApiService] Response data received:');
      console.log('[ApiService] - Session ID:', data.sessionId);
      console.log('[ApiService] - Suggested tests count:', data.suggestedTests?.length || 0);
      console.log('[ApiService] - AI Summary:', data.aiSummary ? 'present' : 'missing');
      console.log('[ApiService] - Repo:', data.repo ? 'present' : 'missing');
      
      return data;
    } catch (error: any) {
      console.error('[ApiService] Error in analyzeFiles:', error);
      throw error;
    }
  }
  ...
  async runTests(sessionId: string, testCaseIds: string[], branch?: string, author?: string): Promise<{
    ...
    try {
      return await this.request<{ runId: string; status: string; total: number }>('/execution/run', {
        method: 'POST',
        body: JSON.stringify({
          sessionId,
          testCaseIds,
   ...
   * Lấy test case code
   */
  async getTestCaseCode(testCaseId: string, sessionId: string): Promise<{ code: string }> {
    return this.request<{ code: string }>(`/execution/test-case/${testCaseId}?sessionId=${sessionId}`);
  }
